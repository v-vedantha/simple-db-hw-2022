...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        // Check if the pid corresponds to a leaf
        if (pid.pgcateg() == BTreePageId.LEAF) {
            // If so, lock the leaf page with the given permissions and return it
        ...
        // Use the iterator to iterate through the children pages
        Iterator<BTreeEntry> it = internalPage.iterator();
        BTreeEntry entry = null;

        while(it.hasNext()) {
            entry = it.next();
            // If the field is null, return the left-most child page
            if (f == null) {
                return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
            }
            // If the field is less than the current entry, return the left child page
            if (f.compare(Op.LESS_THAN_OR_EQ, entry.getKey())) {
                return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
            }
        }

        // if you find nothing, return from the right most page
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        // Get the tuples from the old leaf page
        Iterator<Tuple> it = page.reverseIterator();
        // get the number of tuples
        int numTuples = page.getNumTuples();

        // Move the second half of tuples to the new page
        ...
        Field middleKey = middleTuple.getField(keyField);

        BTreeEntry entry = new BTreeEntry(middleKey, page.getId(), newLeafPage.getId());
        parent.insertEntry(entry);

        // Edit the siblings
        // Set the following page's left id to newLeafPage
        if (page.getRightSiblingId() != null) {
            BTreeLeafPage followingPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
            followingPage.setLeftSiblingId(newLeafPage.getId());
        }
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // Get the entries from the old internal page
        Iterator<BTreeEntry> it = page.reverseIterator();
        // get the number of entries
        int numEntries = page.getNumEntries();
        // Move the second half of the tuples to the new page
        ...
        BTreeEntry entry = new BTreeEntry(middleKey, page.getId(), newInternalPage.getId());
        parent.insertEntry(entry);
        updateParentPointers(tid, dirtypages, parent);
        updateParentPointers(tid, dirtypages, newInternalPage);
        System.out.println("Parent id: " + parent.getId());
        // Add the pages to dirtypages
        dirtypages.put(page.getId(), page);
    ...
    private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        if (parentId.pgcateg() == BTreePageId.ROOT_PTR) {
            ...
            making_root = true;
            System.out.println("Making a new root1");
        } else {
        ...
        if (parent.getNumEmptySlots() == 0) {
            if (making_root)
            {
                System.out.println("Making a new root");
            }
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        for (int i = 0; i < numToSteal; i++) {
            ...
            if (isRightSibling) {
                ...
                sibling.deleteTuple(t);
                // Turns out we don't need to do anything fancy here just inserting the tuple is enough
                // since insert sorts
                page.insertTuple(t);
            }
        }
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry first = it.next();
		BTreeEntry last = new BTreeEntry(parentEntry.getKey(), first.getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(last);

        // Then just steal the rest
		for(int i = 0; i < numSteal; ++i, first = it.next())
		{
			leftSibling.deleteKeyAndRightChild(first);
			page.insertEntry(first);
		}


        // Delete one over the end.
		leftSibling.deleteKeyAndRightChild(first);
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry first = it.next();
		BTreeEntry last = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), first.getLeftChild());
		page.insertEntry(last);
    ...
    public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // Set the left page sibling to the right psge
		leftPage.setRightSiblingId(rightPage.getRightSiblingId());

        // Set the next page sibling id to the left page
        ...
        //  Empty out the right page
		setEmptyPage(tid, dirtypages, rightPage.getId().getPageNumber());

        // Delete the parent entry for the next element
    ...
    public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // Do the trick again where we "flip" the first one to the top
		BTreeEntry last = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(last);

        // Copy everything else overk`
		while(it.hasNext())
		{
			BTreeEntry entry = it.next();
